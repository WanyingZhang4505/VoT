...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        if configs.llm_model == 'Doc2Vec':
            ...
            if configs.llm_model == 'LLAMA2':
                ...
                self.gpt2_config = GPT2Config.from_pretrained('language_model/openai-community/gpt2-medium')
                ...
                try:
                    self.llm_model = GPT2Model.from_pretrained(
                        'language_model/openai-community/gpt2-medium',
                        trust_remote_code=True,
                        local_files_only=True,
                        config=self.gpt2_config,
                    )
                ...
                self.gpt2_config = GPT2Config.from_pretrained('language_model/openai-community/gpt2-large')
                ...
                try:
                    self.llm_model = GPT2Model.from_pretrained(
                        'language_model/openai-community/gpt2-large',
                        trust_remote_code=True,
                        local_files_only=True,
                        config=self.gpt2_config,
                    )
                ...
                self.gpt2_config = GPT2Config.from_pretrained('language_model/openai-community/gpt2-xl')
                ...
                try:
                    self.llm_model = GPT2Model.from_pretrained(
                        'language_model/openai-community/gpt2-xl',
                        trust_remote_code=True,
                        local_files_only=True,
                        config=self.gpt2_config,
                    )
                ...
                self.bert_config = BertConfig.from_pretrained('language_model/google-bert/bert-base-uncased')
    ...
    def vali(self, vali_data, vali_loader, criterion, all_metric=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                text_dec_inp = torch.zeros((prompt_emb.shape[0], self.args.pred_len, self.text_embedding_dim)).to(self.device)
    ...
    def train(self, setting):
        ...
        train_data.get_all_embeddings()
        vali_data.get_all_embeddings()
        test_data.get_all_embeddings()
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(test_loader):
                ...
                if self.Doc2Vec==False:
                    ...
                    input_ids = tokenized_output['input_ids'].view(batch_x.shape[0], self.args.seq_len, language_max_len).to(self.device)
                    attn_mask = tokenized_output['attention_mask'].view(batch_x.shape[0], self.args.seq_len, language_max_len).to(self.device)
